...
import { catchError, map, tap, toArray, take } from 'rxjs/operators';
...
export class HeroService {
  ...
  getHeroes(): Observable<Hero[]> {
    return this.httpClient.get<Hero[]>(this.heroesUrl)
      .pipe(
        tap(_ => this.log('fetched heroes')), // first func
        catchError(this.handleError<Hero[]>('getHeroes', [])) // second func
      );
  }
  ...
  addHero(hero: Hero): Observable<Hero> {
    return this.httpClient.post(this.heroesUrl, hero, this.httpOptions).pipe(
      tap((newHero: Hero) => this.log(`added hewo w/ id=${newHero.id}`)),
